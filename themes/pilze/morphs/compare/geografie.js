/**
 * GEOGRAFIE v2.0 - Compare-Morph für geografische Perspektive
 * 
 ...
 * 3. FUNDORTE & STATISTIK (Fundstatistik, Datenquellen, Räumliche Analyse)
...
export function compareGeografie(items, perspektive, config = {}) {
  debug.morphs('compareGeografie v2.0', { items: items.length });
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 8: RÄUMLICHE STATISTIK
  // ═══════════════════════════════════════════════════════════════════════════
  
  const hasSpatial = items.some(i => 
    i.data.centroid_lat !== undefined || i.data.areal_extent_km2 || i.data.raeumliche_statistik
  );
  
  if (hasSpatial) {
    addGroupHeader(sections, '📐 Räumliche Statistik', 'spatial');
