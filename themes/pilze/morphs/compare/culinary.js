/**
 * culinary - Compare-Morph für culinarye Perspektive v2.0
 * 
 ...
 */
export function compareculinary(items, perspektive, config = {}) {
  debug.morphs('compareculinary', { items: items.length });
  
  const skipFelder = config.skipFelder || null;
  
  const container = document.createElement('div');
  container.className = 'compare-perspektive compare-culinary';
  ...
  header.innerHTML = `
    ...
    <span class="perspektive-name">${perspektive.name || 'culinary'}</span>
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 1: safety & BEWERTUNG
  // ═══════════════════════════════════════════════════════════════════════════
  
  addGroupHeader(sections, '🛡️ safety & Bewertung', 'safety');
  ...
  if (hasTradition) {
    addGroupHeader(sections, '🌍 Tradition & culture', 'culture');
    ...
    addSection(sections, items, 'traditionen', 'cultureelle Verwendung',
      perspektive.farben?.[3], skipFelder, compareObject);
