/**
 * INTERAKTIONEN v2.0 - Compare-Morph für Interaktions-Perspektive
 * 
 ...
 * 2. MULTI-SPECIES INTERAKTIONEN
 * 3. CHEMISCHE ÖKOLOGIE
 * 4. MIKROBIOM
 * 5. WIRT-INTERAKTIONEN
...
export function compareInteraktionen(items, perspektive, config = {}) {
  debug.morphs('compareInteraktionen v2.0', { items: items.length });
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 2: MULTI-SPECIES INTERAKTIONEN
  // ═══════════════════════════════════════════════════════════════════════════
  
  const hasMultiSpecies = items.some(i => 
    i.data.multispecies_interaktionen?.length > 0 || i.data.interaktions_typ
  );
  
  if (hasMultiSpecies) {
    addGroupHeader(sections, '🤝 Multi-Species Interaktionen', 'multispecies');
    
    addSection(sections, items, 'multispecies_interaktionen', 'Multi-Species Interaktionen',
  ...
  const hasChemEcol = items.some(i => 
    i.data.chemische_oekologie?.length > 0 || i.data.allelopathie ||
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 5: WIRT-INTERAKTIONEN
  // ═══════════════════════════════════════════════════════════════════════════
  
  const hasWirtInteraktion = items.some(i => 
    i.data.wirt_interaktionen?.length > 0 || i.data.symbionten?.length > 0 ||
    i.data.parasiten?.length > 0 || i.data.konkurrenten?.length > 0
  );
  
  if (hasWirtInteraktion) {
    addGroupHeader(sections, '🔄 Wirt-Interaktionen', 'host-interactions');
