/**
 * cultivation - Compare-Morph für cultivation-Perspektive v2.0
 * 
...
/**
 * Rendert economylichkeits-Vergleich
 */
 ...
 */
export function comparecultivation(items, perspektive, config = {}) {
  debug.morphs('comparecultivation', { items: items.length });
  
  const skipFelder = config.skipFelder || null;
  
  const container = document.createElement('div');
  container.className = 'compare-perspektive compare-cultivation';
  ...
  header.innerHTML = `
    ...
    <span class="perspektive-name">${perspektive.name || 'cultivation'}</span>
  ...
  if (ernteItems.length > 0) {
    const section = createSectionIfNew('ernte_stats', 'Ernte-statistics', perspektive.farben?.[0], skipFelder);
    if (section) {
  ...
  const problemsSection = createSectionIfNew('cultivation_problems', 'cultivationprobleme', perspektive.farben?.[2], skipFelder);
  if (problemsSection) {
  ...
  // 16. Economics
  const economicsSection = createSectionIfNew('economics', 'economylichkeit', perspektive.farben?.[3], skipFelder);
  if (economicsSection) {
