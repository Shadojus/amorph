/**
 * BESTIMMUNG - Compare-Morph für Identifikation/Determinierung
 * 
 * VOLLSTÄNDIGES BESTIMMUNGSSYSTEM in 10 Hauptbereichen:
 ...
 */
export function compareBestimmung(items, perspektive, config = {}) {
  debug.morphs('compareBestimmung', { items: items.length });
  
  const skipFelder = config.skipFelder || null;
  
  const container = document.createElement('div');
  container.className = 'compare-perspektive compare-bestimmung';
  ...
  header.innerHTML = `
    ...
    <span class="perspektive-name">${perspektive.name || 'Bestimmung'}</span>
  ...
  const hasChemie = items.some(i => 
    i.data.koh_hut || i.data.melzer_sporen || i.data.feso4_fleisch
  );
  
  if (hasChemie) {
    addGroupHeader(sections, '🧪 Chemische Reaktionen', 'chemie');
  ...
  addGroupHeader(sections, '🌲 Ökologie', 'oekologie');
  ...
  addSection(sections, items, 'sichere_merkmale', 'Sichere Merkmale',
    perspektive.farben?.[2], skipFelder, compareList);
  addSection(sections, items, 'bestimmungs_sicherheit_level', 'Sicherheitslevel',
    perspektive.farben?.[3], skipFelder, compareTag);
  
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 8: BESTIMMUNGSHILFEN
  // ═══════════════════════════════════════════════════════════════════════════
  
  addGroupHeader(sections, '📚 Bestimmungshilfen', 'hilfen');
  ...
  addSection(sections, items, 'minimale_merkmalskombination', 'Minimale Kombination',
    perspektive.farben?.[2], skipFelder, compareList);
  addSection(sections, items, 'bestimmungsliteratur', 'Literatur',
  ...
  if (hasFund) {
    ...
    addSection(sections, items, 'bestimmung_status', 'Bestimmungs-Status',
      perspektive.farben?.[0], skipFelder, compareTag);
    addSection(sections, items, 'bestimmt_durch', 'Bestimmt durch',
      perspektive.farben?.[0], skipFelder, compareText);
    addSection(sections, items, 'bestimmungs_methode', 'Methode',
      perspektive.farben?.[0], skipFelder, compareTag);
    addSection(sections, items, 'bestimmungs_konfidenz', 'Konfidenz (%)',
...
export default compareBestimmung;
