/**
 * research - Compare-Morph für researchs-Perspektive (v2.0)
 * 
 * VOLLSTÄNDIG ÜBERARBEITET - Alle Felder sind FLACH
 * 
 * Übersichtlich gruppiert in 8 Hauptbereiche:
 * 1. researchSAKTIVITÄT
 * 2. PUBLIKATIONSTREND
 * 3. researchSGRUPPEN & THEMEN  
 * 4. PATENTE
 * 5. BIOTECHNOLOGIE
 ...
 */
export function compareresearch(items, perspektive, config = {}) {
  debug.morphs('compareresearch v2.0', { items: items.length });
  
  const skipFelder = config.skipFelder || null;
  
  const container = document.createElement('div');
  container.className = 'compare-perspektive compare-research';
  ...
  header.innerHTML = `
    ...
    <span class="perspektive-name">${perspektive.name || 'research'}</span>
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 1: researchSAKTIVITÄT
  // ═══════════════════════════════════════════════════════════════════════════
  
  addGroupHeader(sections, '📊 researchsaktivität', 'activity');
  
  // researchs-Aktivität (Enum)
  addSection(sections, items, 'researchs_aktivitaet', 'Aktivitäts-Level',
    perspektive.farben?.[0], skipFelder, compareTag);
  
  // Aktivitäts-Label
  addSection(sections, items, 'researchs_aktivitaet_label', 'Aktivitäts-Label',
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GRUPPE 3: researchSGRUPPEN & THEMEN
  // ═══════════════════════════════════════════════════════════════════════════
  
  const hasGroups = items.some(i => 
    i.data.researchsgruppe_1_name || i.data.thema_1_name
  );
  
  if (hasGroups) {
    addGroupHeader(sections, '🔬 researchsgruppen & Themen', 'groups');
    
    // Gruppe 1
    addSection(sections, items, 'researchsgruppe_1_name', 'Gruppe 1',
      perspektive.farben?.[0], skipFelder, compareText);
    addSection(sections, items, 'researchsgruppe_1_institution', 'Institution 1',
      perspektive.farben?.[0], skipFelder, compareText);
    addSection(sections, items, 'researchsgruppe_1_land', 'Land 1',
      perspektive.farben?.[0], skipFelder, compareTag);
    addSection(sections, items, 'researchsgruppe_1_schwerpunkt', 'Schwerpunkt 1',
      perspektive.farben?.[0], skipFelder, compareText);
    addSection(sections, items, 'researchsgruppe_1_publikationen', 'Publikationen Gr.1',
      perspektive.farben?.[1], skipFelder, compareBar);
    addSection(sections, items, 'researchsgruppe_1_h_index', 'H-Index Gr.1',
      perspektive.farben?.[1], skipFelder, compareBar);
    
    // Gruppe 2+3 (Kurzform)
    addSection(sections, items, 'researchsgruppe_2_name', 'Gruppe 2',
      perspektive.farben?.[1], skipFelder, compareText);
    addSection(sections, items, 'researchsgruppe_2_land', 'Land 2',
      perspektive.farben?.[1], skipFelder, compareTag);
    addSection(sections, items, 'researchsgruppe_3_name', 'Gruppe 3',
  ...
  if (hasFuture) {
    ...
    addSection(sections, items, 'zukunftspotential_markt_attraktivitaet', 'Markt-Attraktivität',
      perspektive.farben?.[0], skipFelder, (mapped, cfg) => 
        compareBar(mapped, { ...cfg, max: 10 }));
    addSection(sections, items, 'zukunftspotential_researchs_momentum', 'researchs-Momentum',
      perspektive.farben?.[0], skipFelder, (mapped, cfg) => 
        compareBar(mapped, { ...cfg, max: 10 }));
    ...
    // researchslücken
    addSection(sections, items, 'researchsluecke_1_bereich', 'Lücke 1 Bereich',
      perspektive.farben?.[2], skipFelder, compareText);
    addSection(sections, items, 'researchsluecke_1_prioritaet', 'Lücke 1 Priorität',
      perspektive.farben?.[2], skipFelder, compareTag);
    addSection(sections, items, 'researchsluecke_1_aufwand', 'Lücke 1 Aufwand',
      perspektive.farben?.[2], skipFelder, compareTag);
    addSection(sections, items, 'researchsluecke_2_bereich', 'Lücke 2 Bereich',
      perspektive.farben?.[3], skipFelder, compareText);
